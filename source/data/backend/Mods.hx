...
class Mods
{
	...
	inline public static function getModsListPath():String
	{
		#if mobile
		return StorageUtil.getStorageDirectory() + 'modsList.txt';
		#else
		return 'modsList.txt';
		#end
	}
	...
	inline public static function parseList():ModsList {
		...
		#if MODS_ALLOWED
		try {
			var listPath = getModsListPath();
			if (FileSystem.exists(listPath))
			{
				for (mod in CoolUtil.coolTextFile(listPath))
				{
	...
	private static function updateModList()
	{
		...
		#if mobile
		var directory = haxe.io.Path.directory(listPath);
		if (!FileSystem.exists(directory))
			FileSystem.createDirectory(directory);
		#end

		File.saveContent(listPath, fileStr);
		updatedOnState = true;
		#end
	}

...
class CustomDPad extends MobileInputManager implements IMobileControls
{
	...
	public function new(X:Float, Y:Float)
	{
		...
		padBG = new TouchButton(X + (offset - 28), Y + (offset + 35), []);
		...
		arrows = new TouchButton(dPad.x + (offset - 15), dPad.y + (offset - 15), []);
		if (customFrames != null)
		{
		...
		buttonUp = createCustomButton(X + 90, Y + 65, 0.5, 0.5, 'null0000', [MobileInputID.GAME_UP, MobileInputID.UP, MobileInputID.UP2], customFrames);
		buttonRight = createCustomButton(X + 170, Y + 150, 0.5, 0.5, 'null0000', [MobileInputID.GAME_RIGHT, MobileInputID.RIGHT, MobileInputID.RIGHT2], customFrames);
		...
		pausePad = new TouchButton(X + (FlxG.width - 140), Y - 340, []);
		if (customFrames != null)
		{
